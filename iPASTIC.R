...
(function()
{
  ...
  getpca <- function(df, iscov)
  {
    ...
    if (!iscov)
    {
      ...
      scoresignchangevec <-
        getSignChangingVec(as.matrix(pca$rotation))[col(pca$x)]
    ...
    else
    {
      ...
      scoresignchangevec <-
        getSignChangingVec(as.matrix(prcomp(df, scale = FALSE)$rotation))[col(scores)]
    ...
    scores <-
      scores[, 1:min(nrow(scores), length(signChangingFlags))]
    
    eigenvals <- t(as.data.frame(eigenvals))
    importancedf <- summary(pca)$importance[2:3, ]
    eigenvals <-
      rbind(eigenvals[, 1:min(ncol(importancedf), ncol(eigenvals))], importancedf[, 1:min(ncol(importancedf), ncol(eigenvals))])
    
    colnames(eigenvals) <- getpcLabels(ncol(eigenvals))
    rownames(eigenvals) <-
      c("Eigenvalue", "Variability (%)", "Cumulative %")
    ...
    pca$rotation <-
      pca$rotation * getSignChangingVec(pca$rotation[,!apply(pca$rotation , 2 , function(x)
        all(is.na(x)))])[col(pca$rotation)]
    pca$x <-
      pca$x * getSignChangingVec(pca$x[,!apply(pca$x , 2 , function(x)
        all(is.na(x)))])[col(pca$x)]
    
    return(
      list(
        eigenvals = eigenvals,
        eigenvecs = eigenvecs * signChangingFlags[col(eigenvecs)],
        loadings = loadings * signChangingFlags[col(loadings)],
        contributions = contributions[, 1:length(signChangingFlags)],
        scores = scores,
        pca_obj = pca
      )
    )
  ...
  Calculate <<- function(table_original)
  {
    ...
    {
      warning(
        "There are more variables than observations in output for dataset provided for principal component analysis."
      )
    }
    ...
    output <-
      list(
        ...
        ranks = ranks_df,
        correlations = list(
          pearson = cor(data.matrix(stats_df[-4][, 2:length(stats_df[-4])])),
          spearman = cor(data.matrix(ranks_df[, 2:(length(ranks_df) - 3)]))
        ),
        pca = list(
